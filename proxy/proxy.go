		...
		go func(source tunnel.Server) {
			for {
				...
				go func(inbound tunnel.Conn) {
					...
					if inbound.Metadata().TargetAddr == nil {
						log.Error(common.NewError("proxy failed to get metadata"))
						return
					}

					log.Debug("proxy ", inbound.Metadata().RemoteAddr, "=>", inbound.Metadata().TargetAddr)
					outbound, err := p.sink.DialConn(inbound.Metadata().TargetAddr, nil)
