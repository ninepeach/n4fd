...
func (r *Metadata) ReadFrom(rr io.Reader) error {
	...
	r.TargetAddr = new(Address)
	err = r.TargetAddr.ReadFrom(rr)
	if err != nil {
...
func (r *Metadata) WriteTo(w io.Writer) error {
	...
	if err := r.TargetAddr.WriteTo(buf); err != nil {
		return err
	}
	// use tcp by default
	r.TargetAddr.NetworkType = "tcp"
	_, err := w.Write(buf.Bytes())
	return err
}

func (r *Metadata) Network() string {
	return r.TargetAddr.Network()
}

func (r *Metadata) String() string {
	return r.TargetAddr.String()
}
...
func (a *Address) ReadFrom(r io.Reader) error {
	...
	case DomainName:
		...
		// the browser uses IP as a domain name sometimes
		host := buf[0:length]
