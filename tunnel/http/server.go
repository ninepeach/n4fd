...
func (s *Server) acceptLoop() {
    for {
        ...
        go func(conn net.Conn) {
            ...
            if err != nil {
                log.Error(common.NewError("not a valid http request").Base(err).Error())
                return
            }
            ...
            if strings.ToUpper(req.Method) == "CONNECT" { // CONNECT
                ...
                if err != nil {
                    log.Error(common.NewError("invalid http dest address").Base(err).Error())
                    conn.Close()
                    return
                }
                ...
                for {
                    ...
                    log.Debug("http dest %s", addr.String())
                    ...
                    if err != nil {
                        log.Error(common.NewError("http failed to write http request").Base(err).Error())
                        return
                    }
                    ...
                    if err != nil {
                        log.Error(common.NewError("http failed to read http response").Base(err).Error())
                        return
                    }
                    err = resp.Write(conn) // send the response back to the local
                    if err != nil {
                        log.Error(common.NewError("http failed to write the response back").Base(err).Error())
                        return
                    }
                    newConn.Close()
                    req.Body.Close()
                    ...
                    if err != nil {
                        log.Error(common.NewError("http failed to the read request from local").Base(err).Error())
                        return
                    }
                }
            }
