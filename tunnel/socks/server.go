...
func (s *Server) packetDispatchLoop() {
    for {
        ...
        log.Debug("socks recv udp packet from %s", src)
        s.mappingLock.RLock()
        ...
        if !found {
            ...
            go func(conn *PacketConn) {
                ...
                for {
                    ...
                    case info := <-conn.output:
                        ...
                        if err != nil {
                            log.Error("socks failed to respond packet to %s", src)
                            return
                        }
                        log.Debug("socks respond udp packet to %s metadata %s", src, info.metadata)
            ...
            s.packetChan <- conn
            log.Info("socks new udp session from %s", src)
        }
        r := bytes.NewBuffer(buf[3:n])
        address := new(tunnel.Address)
        if err := address.ReadFrom(r); err != nil {
            log.Error(common.NewError("socks failed to parse incoming packet").Base(err).Error())
            continue
        }
...
func (s *Server) acceptLoop() {
    for {
        ...
        if err != nil {
            log.Error(common.NewError("socks accept err").Base(err).Error())
            return
        }
        go func(conn net.Conn) {
            newConn, err := s.handshake(conn)
            if err != nil {
                log.Error(common.NewError("socks failed to handshake with client").Base(err).Error())
                return
            }
            log.Info("socks connection from %s metadata %s", conn.RemoteAddr(), newConn.metadata.String())
            switch newConn.metadata.Command {
            case Connect:
                if err := s.connect(newConn); err != nil {
                    log.Error(common.NewError("socks failed to respond CONNECT").Base(err).Error())
                    newConn.Close()
                    return
                }
                s.connChan <- newConn
            ...
            case Associate:
                ...
                if err := s.associate(newConn, associateAddr); err != nil {
                    log.Error(common.NewError("socks failed to respond to associate request").Base(err).Error())
                    return
                }
                buf := [16]byte{}
                newConn.Read(buf[:])
            ...
            default:
                log.Error(common.NewError(fmt.Sprintf("unknown socks command %d", newConn.metadata.Command)).Error())
                newConn.Close()
            }
        }(conn)
    }
